...
def split_meth_dataframe(df):
    df = pd.read_csv(df, delimiter="\t")
    
    
    total_columns = len(df.columns)
    split_index = math.ceil(total_columns * 0.8)

    #add missing cpg labels and 0.5 for all methylation values per sample 
    ...
    test_df = df.iloc[:, split_index:].round()
    labels_column = df.iloc[:, 0:1]
    test_df = pd.concat([labels_column, test_df], axis=1)

    
    return training_df, test_df
    #return test_df
    
def get_clock_preds(df):
    df = pd.DataFrame(df)
    altum = get_altum_age(df)
    altum = pd.DataFrame(altum)
    horvath = get_horvath_age(df)
    horvath = pd.DataFrame(horvath)
    hannum = get_hannum_age(df)
    hannum = pd.DataFrame(hannum)
    pheno = get_pheno_age(df)
    pheno = pd.DataFrame(pheno)
    horvath_sb = get_horvath_sb_age(df)
    horvath_sb = pd.DataFrame(horvath_sb)
    #grim =  get_grim_age(df)
    #grim = pd.DataFrame(grim)
    return altum, horvath, hannum, pheno, horvath_sb

def split_anno_dataframe(df):
    df = pd.read_csv(df, delimiter="\t")
    df = df.iloc[9:10]
    df = pd.DataFrame(df, index=None)
    df.to_csv("test.csv", index=False)  # Set index=False to exclude row numbers
    ...
    training_df = df.iloc[:, :split_index]
    test_df = df.iloc[:, split_index:].round()
    training = training_df.values.flatten().tolist()
    training = training[1:]
    training_data = []
    for i in range(len(training)):
        training_data.append(int(training[i].split('-')[0])+5)
        
    test = test_df.values.flatten().tolist()
    test = test[1:]
    test_data = []
    for i in range(len(test)):
        test_data.append(int(test[i].split('-')[0])+5)
    training_data = pd.DataFrame(training_data)
    test_data = pd.DataFrame(test_data)
    return training_data, test_data
    #return test_df, training_df

age_training, age_test = split_anno_dataframe("/Users/yashagarwal/Downloads/GTExData/GTEx_anno/GTEx_Breast.anno.csv")
meth_training, meth_test = split_meth_dataframe("/Users/yashagarwal/Downloads/GTExData/GTEx_methylation/GTEx_Breast.meth.csv")
altum, horvath, hannum, pheno, horvath_sb = get_clock_preds(meth_training)

training_dfs = [altum, horvath, hannum, pheno, horvath_sb]
#print(training_dfs)


x_train = np.vstack([df.values.flatten().tolist() for df in training_dfs])  # Input values from training DataFrames
y_train = age_training.values  # Target values
x_train = x_train.T
#print(x_train)
#print(x_train.shape)
#print(y_train)
...
altum_test, horvath_test, hannum_test, pheno_test, horvath_sb_test = get_clock_preds(meth_test)
test_dfs = [altum_test, horvath_test, hannum_test, pheno_test, horvath_sb_test]

x_test = np.vstack([df.values.flatten().tolist() for df in test_dfs])
